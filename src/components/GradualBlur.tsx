...
const GradualBlur: React.FC<GradualBlurProps> = props => {
  ...
  const containerStyle = useMemo(() => {
    ...
    const baseStyle: React.CSSProperties = {
      ...
      transition: config.animated ? `opacity ${config.duration} ${config.easing}` : 'opacity 0.4s ease-out',
      zIndex: isPageTarget ? (config.zIndex || 40) : config.zIndex,
      ...config.style
    };
...
const GradualBlurMemo: React.FC<GradualBlurProps> & {
  PRESETS: typeof PRESETS;
  CURVE_FUNCTIONS: typeof CURVE_FUNCTIONS;
} = React.memo(GradualBlur) as any;
